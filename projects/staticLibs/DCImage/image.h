...
namespace DC
{
	...
	public:
		...
		bool load(const String& strFilename, bool bFlipForOpenGL = false);
		...
		bool loadInfo(const String& strFilename, int& iWidth, int& iHeight, int& componentCount);

		// Save image as BMP file to disk.
		// Throws exception if image contains no data or saving fails.
		void saveAsBMP(const String& strFilename, bool bFlipOnSave = false) const;

		// Save image to JPG file to disk
		// Throws exception if image contains no data or saving fails.
		void saveAsJPG(const String& strFilename, bool bFlipOnSave = false, int iQuality = 100) const;

		// Save image to PNG file to disk
		// Throws exception if image contains no data or saving fails.
		void saveAsPNG(const String& strFilename, bool bFlipOnSave = false) const;

		// Save image to TGA file to disk
		// Throws exception if image contains no data or saving fails.
		void saveAsTGA(const String& strFilename, bool bFlipOnSave = false) const;
		...
		void copyRectTo(Image& destImage, int iSrcPosX, int iSrcPosY, int iSrcWidth, int iSrcHeight, int iDestPosX, int iDestPosY) const;
		...
		// If this image contains no data, an exception occurs.
		void normalmap(Image& outputImage, float fScale = 1.0f) const;
...
//		void fillFromOpenGL(const Vector2f& vSourcePosTLCorner = Vector2f());
		...
		Colour getColourWheelColour(unsigned int iPositionX, unsigned int iPositionY, unsigned int iWidthAndHeightOfImage, unsigned char ucBrightness);

		// Creates this image with the specified values and draws a gradient between the two given colours.
		void drawGradient(unsigned int iWidth, unsigned int iHeight, unsigned int iNumChannels, const Colour& colour0, const Colour& colour1);

		// Creates this image with the specified values and draws a circle with the given colour values
		void drawCircle(unsigned int iWidthAndHeightOfImage, const Colour& colourInner, const Colour& colourOuter);
	private:
		unsigned char* data;
	...
	inline void Image::getPixel(int iX, int iY, unsigned char& r, unsigned char& g, unsigned char& b, unsigned char& a) const
	{
		if (iX >= _miWidth)
			return;
		if (iY >= _miHeight)
